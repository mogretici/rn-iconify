...
### Automatic Accessibility (v3.0)

In v3.0, `IconRenderer` automatically:

- Sets `accessibilityRole="image"` on every icon
- Generates an accessibility label from the icon name (e.g., `mdi:home` → "home icon") when `AccessibilityProvider` is present
- Disables animations when `shouldDisableAnimations()` returns `true` (respects system Reduce Motion preference)

These features work out of the box — no manual code needed. The examples below show how to customize behavior beyond the defaults.
