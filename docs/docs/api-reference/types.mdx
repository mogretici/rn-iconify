...
### Props for a specific icon set

There is no `MdiProps` or `HeroiconsProps`. Every icon component takes
`IconProps`, parameterised by that set's name type:

```tsx
import type { IconProps, MdiIconName, HeroiconsIconName } from 'rn-iconify';

type MdiProps = IconProps<MdiIconName>;
type HeroiconsProps = IconProps<HeroiconsIconName>;
```

Which is what the components are built from, so the two stay in step:

```tsx
function SaveButton({ name, size }: IconProps<MdiIconName>) {
  return <Mdi name={name} size={size} />;
}
```
...
```tsx
import { ExplorerContext } from 'rn-iconify/dev';
...
```tsx
import { fetchIconsBatch, type BatchFetchResult } from 'rn-iconify';

const result: BatchFetchResult = await fetchIconsBatch(['mdi:home', 'mdi:account']);
...
```tsx
import { getNativeIconify, isNativeModuleAvailable, type PrefetchResult } from 'rn-iconify';

if (isNativeModuleAvailable()) {
  const native = getNativeIconify();
  const result: PrefetchResult = await native.prefetchIcons([
    'mdi:home',
    'mdi:user',
    'mdi:settings',
  ]);
...
```tsx
import { getNativeIconify, isNativeModuleAvailable, type CacheStats } from 'rn-iconify';

if (isNativeModuleAvailable()) {
  const stats: CacheStats = await getNativeIconify().getCacheStats();
...
```tsx
import { getNativeIconify, isNativeModuleAvailable, type ModuleConstants } from 'rn-iconify';

if (isNativeModuleAvailable()) {
  const constants: ModuleConstants = getNativeIconify().getConstants();
...
Interface for the native module. Obtained from `getNativeIconify()` when the native module is installed.
...
```tsx
import { getNativeIconify, isNativeModuleAvailable } from 'rn-iconify';

// Check availability before using
if (isNativeModuleAvailable()) {
  // Prefetch critical icons on app start
  await getNativeIconify().prefetchIcons(['mdi:home', 'mdi:menu', 'mdi:close']);

  // Check cache status
  const isCached = getNativeIconify().isCached('mdi:home');

  // Get stats
  const stats = await getNativeIconify().getCacheStats();

  // Clear cache when needed
  await getNativeIconify().clearCache();
}
```

:::info
The native module is optional. If not installed, `getNativeIconify().isAvailable` will be `false` and the library will use JavaScript-based caching instead.
:::
